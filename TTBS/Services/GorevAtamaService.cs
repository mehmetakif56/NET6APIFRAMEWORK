...
namespace TTBS.Services
{
    public interface IGorevAtamaService
    {
        ...
        void UpdateStenoGorevTamamla(Guid birlesimId, ToplanmaTuru toplanmaTuru, int satirNo);
        void UpdateStenoGorevAraVerme(Guid birlesimId, ToplanmaTuru toplanmaTuru, int satirNo);
    ...
    public class GorevAtamaService : BaseService, IGorevAtamaService
    {
        ...
        public List<GorevAtamaModel> BirlesimIptalHesaplama(List<GorevAtamaModel> atamaList)
        {
            //atamaList.ForEach(x =>
            //{
            ...
            //});
            var gorevBasTarihi = atamaList.Where(x => x.GorevBasTarihi != DateTime.MinValue).FirstOrDefault().GorevBasTarihi.Value;
            ...
            foreach (var type in conventionTypes)
            {
                if (type.Equals(ToplanmaTuru.GenelKurul))
                {
                    ...
                    if (gkBirlesim != null && gkBirlesim.Any())
                    {
                        ...
                        if (gkModel != null && gkModel.Any())
                        {
                            ...
                            gkModel.ForEach((data) =>
                            {
                                ...
                                if (steno.GrupId.Equals(grupId) && gidenGrupPasif == DurumStatu.Hayır && data.GorevBasTarihi >= gidenGrupTarih)
                                {
                                    ...
                                    //extractedGK.Add(_mapper.Map<GorevAtamaGenelKurul>(data));
                                }
                            });
                        ...
                        var yeniGorevList = BirlesimIptalHesaplama(GetGorevAtamaByBirlesimId(gkBirlesim.FirstOrDefault().Id, ToplanmaTuru.GenelKurul).Where(w => w.StenoGorevTuru == 0).ToList());
                        _gorevAtamaGKRepo.Update(_mapper.Map<List<GorevAtamaGenelKurul>>(yeniGorevList));
                        //UpdateBirlesimGorevAtama(gkBirlesim.FirstOrDefault().Id, gkBirlesim.FirstOrDefault().TurAdedi);
                    }
                }
                else if (type.Equals(ToplanmaTuru.Komisyon))
                {
                    var commisionModel = _gorevAtamaKomRepo.Get(x => x.Birlesim.ToplanmaDurumu != ToplanmaStatu.Iptal && x.Birlesim.ToplanmaDurumu != ToplanmaStatu.Tamamlandı && x.GorevStatu != GorevStatu.Tamamlandı).OrderBy(x => x.SatırNo).ToList();
        ...
        public void DeleteGorevByBirlesimIdAndStenoId(Guid birlesimId, List<Guid> stenografIds, ToplanmaTuru toplanmaTuru)
        {
            var birlesim = _birlesimRepo.GetById(birlesimId);
            if (birlesim != null && (birlesim.ToplanmaDurumu == ToplanmaStatu.Oluşturuldu || birlesim.ToplanmaDurumu == ToplanmaStatu.Planlandı))
            {
                ...
                stenografIds.ForEach(y =>
                {
                    var gorevSteno = gorevler.Where(x => x.StenografId == y);
                    if (gorevSteno != null && gorevSteno.Count() > 0)
                    {
                        DeleteGorevAtama(gorevSteno, toplanmaTuru);

                        gorevler = gorevler.Where(x => x.StenografId != y).OrderBy(x => x.GorevBasTarihi).ToList();
        ...
        private void DeleteGorevAtama(IEnumerable<GorevAtamaModel> model, ToplanmaTuru toplanmaTuru)
        {
            ...
            {
                var entity = Mapper.Map<List<GorevAtamaKomisyon>>(model);
        ...
        public void UpdateStenoGorevTamamla(Guid birlesimId, ToplanmaTuru toplanmaTuru, int satirNo)
        {
                    ...
                    try
                    {
                        ...
                        result.Where(x => x.SatırNo <= satirNo && x.GorevStatu != GorevStatu.Iptal).ToList().ForEach(x =>
                        {
                            if (x.StenoIzinTuru.Equals(IzınTuru.Bulunmuyor))
                            {
                                ...
                                if (x.SatırNo == satirNo && x.StenoGorevTuru == StenoGorevTuru.Stenograf)
                                {
        ...
        public void UpdateStenoGorevAraVerme(Guid birlesimId, ToplanmaTuru toplanmaTuru, int satirNo)
        {
        ...
        public string GetKomisyonMinMaxDate(Guid stenoId, DateTime? gorevBasTarih, DateTime? gorevBitisTarih, double sure)
        {
            var result = _gorevAtamaKomRepo.Get(x => x.StenografId == stenoId && x.GorevStatu != GorevStatu.Iptal && x.GorevStatu != GorevStatu.Tamamlandı &&
                                                ...
                                                x.Birlesim.ToplanmaDurumu != ToplanmaStatu.Tamamlandı &&
                                                x.OnayDurumu == true, includeProperties: "Birlesim.Komisyon")
        ...
        public void ApproveStenografKomisyon()
        {
            ...
            if (result != null && result.Count() > 0)
            {
                foreach (var item in result)
                {
                    ...
                    degisenAtamlar.ForEach(x => x.OnayDurumu = true);
