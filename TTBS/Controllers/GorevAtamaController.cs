...
namespace TTBS.Controllers
{
    ...
    public class GorevAtamaController : BaseController<GorevAtamaController>
    {
        ...
        private List<GorevAtamaModel> SetGorevAtama(Birlesim birlesim, Guid oturumId, IEnumerable<Guid> stenoList,double sure,ToplanmaTuru toplanmaTuru, StenoGorevTuru gorevTuru)
        {
            ...
            var gidenGrupSaat = GidenGruoHesaplama(toplanmaTuru, sure);
            int firstRec = 0;
        ...
        private DateTime GidenGruoHesaplama(ToplanmaTuru toplanmaTuru,double sure)
        {
        ...
        private List<GorevAtamaModel> BirlesimKomisyonHesaplama(List<GorevAtamaModel> atamaList)
        {
            ...
            foreach (var item in atamaList)
            {
                if (!string.IsNullOrEmpty(item.KomisyonAd))
                {
                    item.GorevStatu = GorevStatu.Iptal;
                }
                else
        ...
        public IActionResult CreateStenoGorevDonguEkle(Guid birlesimId, Guid oturumId)
        {
            try
            {
                var atamaList = _gorevAtamaService.CreateStenoGorevDonguEkle(birlesimId, oturumId);
        ...
        public IActionResult UpdateGorevDurumById(Guid id, Guid birlesimId, ToplanmaTuru toplanmaTuru)
        {
            try
            {
               var result=_gorevAtamaService.UpdateGorevDurumById(id, birlesimId, toplanmaTuru);
               if(result!=null && result.Count()>0)
               {
                    var hesaplama = BirlesimIptalHesaplama(result);
        ...
        public IActionResult UpdateGorevDurumByBirlesimAndSteno(Guid birlesimId, Guid stenoId, ToplanmaTuru toplanmaTuru)
        {
            try
            {
                var result = _gorevAtamaService.UpdateGorevDurumByBirlesimAndSteno(birlesimId, stenoId, toplanmaTuru);
                if (result != null && result.Count() > 0)
                {
                    var hesaplama = BirlesimIptalHesaplama(result);
                    if(hesaplama!=null && hesaplama.Count>0)
        ...
        private void UpdateGenelKurulKomisyon()
        {
            ...
            if (gkBirlesim != null)
            {
                ...
                if (gkAtama != null && gkAtama.Count()>0)
                {
                    ...
                    var result = BirlesimKomisyonHesaplama(modelList);
